...
interface shipmentType {
 ...
 useEffect(() => {
   ...
   setValue("expected_delivery_date", selectedShipment?.expected_delivery_date);
   setValue("shipping_cost", selectedShipment?.shipping_cost);
   setValue("carrier_name", selectedShipment?.carrier_name);
 }, [selectedShipment]);
 ...
 const handleUpdateShipment = async (data: any) => {
   try {
     console.log(data);
     await axios.put(
       `http://localhost:8800/api/admin-update-shipment?id=${shipmentId}`,
       data,
       config
     );
     const closeButton = document.getElementById("btn-close1");
     closeButton?.click();
     getAllShipments();
   } catch (err: any) {
     console.log(err);
  ...
  const getAllShipments = async () => {
    try {
      let result:any = await axios.get(
        "http://localhost:8800/api/all-shipments",
        config
      );
  ...
  const updateStatus = async () => {
    try {
      let result = await axios.put(
        "http://localhost:8800/api/update-shipment-status",
 ...
 const handleDeleteConfirmation = (shipmentId) => {
   ...
   const deleteShipment = async () => {
     try {
       let result = await axios.delete(
         `http://localhost:8800/api/admin-shipment-delete?id=${shipmentId}`,
         config
       );
       getAllShipments();
     } catch (err) {
            ...
            <div className="modal-body">
              <form onSubmit={handleSubmit(onSubmitHandler)}>
                ...
                <div className="mt-3">
                  ...
                  />
                  {errors.carrier_name && (
                    <p className="text-danger">{errors.carrier_name.message}</p>
                  )}
                </div>
